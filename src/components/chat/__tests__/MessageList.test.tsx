...
// Mock the ToolStatusBadge component
vi.mock("../ToolStatusBadge", () => ({
  ToolStatusBadge: ({ toolName, args }: { toolName: string; args: Record<string, unknown> }) => {
    const command = args?.command as string;
    const path = args?.path as string;
    if (toolName === "str_replace_editor" && command === "create" && path) {
      return <div>Creating {path}</div>;
    }
